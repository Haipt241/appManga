...
import java.io.File
import com.bigberry.comicvn.BuildConfig.APPLICATION_ID as ID
...
internal class UpdateDownloaderReceiver(val context: Context) : BroadcastReceiver() {
    ...
    private fun basicNotification() {
        // Create notification
        with(notification) {
            setContentTitle(context.getString(com.bigberry.comicvn.R.string.app_name))
            setContentText(context.getString(com.bigberry.comicvn.R.string.update_check_notification_download_in_progress))
    ...
    private fun installNotification(path: String) {
        ...
        with(notification) {
            setContentText(context.getString(com.bigberry.comicvn.R.string.update_check_notification_download_complete))
            setSmallIcon(android.R.drawable.stat_sys_download_done)
            setProgress(0, 0, false)
            // Install action
            setContentIntent(NotificationHandler.installApkPendingActivity(context, File(path)))
            addAction(com.bigberry.comicvn.R.drawable.ic_system_update_grey_24dp_img,
                    context.getString(com.bigberry.comicvn.R.string.action_install),
                    NotificationHandler.installApkPendingActivity(context, File(path)))
            // Cancel action
            addAction(com.bigberry.comicvn.R.drawable.ic_clear_grey_24dp_img,
                    context.getString(com.bigberry.comicvn.R.string.action_cancel),
    ...
    private fun errorNotification(url: String) {
        ...
        with(notification) {
            setContentText(context.getString(com.bigberry.comicvn.R.string.update_check_notification_download_error))
            setSmallIcon(android.R.drawable.stat_sys_warning)
            setProgress(0, 0, false)
            // Retry action
            addAction(com.bigberry.comicvn.R.drawable.ic_refresh_grey_24dp_img,
                    context.getString(com.bigberry.comicvn.R.string.action_retry),
