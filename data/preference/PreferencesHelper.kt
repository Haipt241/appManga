package com.bigberry.comicvn.data.preference
...
class PreferencesHelper(val context: Context) {
    ...
    private val defaultDownloadsDir = Uri.fromFile(
            File(Environment.getExternalStorageDirectory().absolutePath + File.separator +
                    context.getString(com.bigberry.comicvn.R.string.app_name), "downloads"))

    private val defaultBackupDir = Uri.fromFile(
            File(Environment.getExternalStorageDirectory().absolutePath + File.separator +
                    context.getString(com.bigberry.comicvn.R.string.app_name), "backup"))

    fun startScreen(): Int {
        return prefs.getInt(Keys.startScreen, 4)
    }
    ...
    fun enabledLanguages() = rxPrefs.getStringSet(Keys.enabledLanguages, setOf("vi"))

    fun username() = prefs.getString(Keys.username, "")

    fun password() = prefs.getString(Keys.password, "")
    ...
    fun isSendUserInfo() = prefs.getBoolean(Keys.isSendUserInfo, false)

    fun setSendUserInfo(isSent: Boolean){
        prefs.edit()
                .putBoolean(Keys.isSendUserInfo, isSent)
                .apply()
    }

    fun getIntervalShowAds() = prefs.getInt(Keys.ads_interval, 0)
    fun setIntervalShowAds(num: Int) = prefs.edit()
            .putInt(Keys.ads_interval, num)
            .apply()
}
