...
import com.bigberry.comicvn.ui.catalogue.filter.*
        ...
        pagerSubscription = pager.results()
                ...
                .map {
                    it.first to it.second.map {
                        networkToLocalManga(it, sourceId)
                    }
                }
                .doOnNext {
                ...
                .map {
                    it.first to it.second.map(::CatalogueItem)
                }
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeReplay(onNext = { view, pair ->
...
//                .sortedBy { "(${it.lang}) ${it.name}" } //+ sourceManager.get(LocalSource.ID) as LocalSource
                .sortedBy { "(${it.id})" } //+ sourceManager.get(LocalSource.ID) as LocalSource
    }
