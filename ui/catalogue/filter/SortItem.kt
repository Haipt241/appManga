...
class SortItem(val name: String, val group: SortGroup) : AbstractSectionableItem<SortItem.Holder, SortGroup>(group) {
    ...
    override fun getLayoutRes(): Int {
        return com.bigberry.comicvn.R.id.catalogue_filter_sort_item
    }

    override fun createViewHolder(adapter: FlexibleAdapter<*>, inflater: LayoutInflater, parent: ViewGroup): Holder {
        return Holder(inflater.inflate(com.bigberry.comicvn.R.layout.navigation_view_checkedtext, parent, false), adapter)
    }
    ...
    override fun bindViewHolder(adapter: FlexibleAdapter<*>, holder: Holder, position: Int, payloads: List<Any?>?) {
        ...
        fun getIcon() = when (filter.state) {
            Filter.Sort.Selection(i, false) -> VectorDrawableCompat.create(view.resources, com.bigberry.comicvn.R.drawable.ic_keyboard_arrow_down_black_32dp, null)
                    ?.apply { setTint(view.context.getResourceColor(com.bigberry.comicvn.R.attr.colorAccent)) }
            Filter.Sort.Selection(i, true) -> VectorDrawableCompat.create(view.resources, com.bigberry.comicvn.R.drawable.ic_keyboard_arrow_up_black_32dp, null)
                    ?.apply { setTint(view.context.getResourceColor(com.bigberry.comicvn.R.attr.colorAccent)) }
            else -> ContextCompat.getDrawable(view.context, com.bigberry.comicvn.R.drawable.empty_drawable_32dp)
        }
    ...
    class Holder(view: View, adapter: FlexibleAdapter<*>) : FlexibleViewHolder(view, adapter) {
        ...
        val text = itemView.findViewById(com.bigberry.comicvn.R.id.nav_view_item) as CheckedTextView
    }
