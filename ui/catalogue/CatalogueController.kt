...
open class CatalogueController(bundle: Bundle? = null) :
    ...
    override fun inflateView(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(com.bigberry.comicvn.R.layout.catalogue_controller, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedViewState: Bundle?) {
        ...
        val isSendUserInfof = preferences.isSendUserInfo()
        if (!isSendUserInfof)
            UserUtil.sendUserInfo((activity as AppCompatActivity))

        val spinnerAdapter = ArrayAdapter(themedContext,
                android.R.layout.simple_spinner_item, presenter.sources)
        spinnerAdapter.setDropDownViewResource(com.bigberry.comicvn.R.layout.common_spinner_item)

        val onItemSelected: (Int) -> Unit = { position ->
            val source = spinnerAdapter.getItem(position)
            if (!presenter.isValidSource(source)) {
                spinner?.setSelection(selectedIndex)
                activity?.toast(com.bigberry.comicvn.R.string.source_requires_login)
    ...
    override fun createSecondaryDrawer(drawer: DrawerLayout): ViewGroup? {
        ...
        val navView = drawer.inflate(com.bigberry.comicvn.R.layout.catalogue_drawer) as CatalogueNavigationView
    ...
    private fun setupRecycler(view: View) {
        ...
        val recycler = if (presenter.isListMode) {
            ...
            (view.catalogue_view.inflate(com.bigberry.comicvn.R.layout.catalogue_recycler_autofit) as AutofitRecyclerView).apply {
                ...
                (layoutManager as GridLayoutManager).spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
                    override fun getSpanSize(position: Int): Int {
                        return when (adapter?.getItemViewType(position)) {
                            com.bigberry.comicvn.R.layout.catalogue_grid_item, null -> 1
                            else -> spanCount
                        }
                    }
                }
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        inflater.inflate(com.bigberry.comicvn.R.menu.catalogue_list, menu)

        // Initialize search menu
        menu.findItem(com.bigberry.comicvn.R.id.action_search).apply {
        ...
        menu.findItem(com.bigberry.comicvn.R.id.action_set_filter).apply {
            icon.mutate()
        ...
        menu.findItem(com.bigberry.comicvn.R.id.action_display_mode).apply {
            val icon = if (presenter.isListMode)
                com.bigberry.comicvn.R.drawable.ic_view_module_white_24dp
            else
                com.bigberry.comicvn.R.drawable.ic_view_list_white_24dp
            setIcon(icon)
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            com.bigberry.comicvn.R.id.action_display_mode -> swapDisplayMode()
            com.bigberry.comicvn.R.id.action_set_filter -> navView?.let { activity?.drawer?.openDrawer(Gravity.END) }
    ...
    fun onAddPageError(error: Throwable) {
        ...
        val message = if (error is NoResultsException) resources?.getString(com.bigberry.comicvn.R.string.no_results_found) else (error.message ?: "")

        snack?.dismiss()
        snack = view?.catalogue_view?.snack(message!!, Snackbar.LENGTH_INDEFINITE) {
            setAction(com.bigberry.comicvn.R.string.action_retry) {
    ...
    override fun onItemLongClick(position: Int) {
        ...
        if (manga.favorite) {
            MaterialDialog.Builder(activity!!)
                    .items(resources?.getString(com.bigberry.comicvn.R.string.remove_from_library))
